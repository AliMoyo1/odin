...
- 2026-07-11: PLAN-02 complete (auth, 8/8 tests). PLAN-03 complete (core API, WS, 8/8 tests; added POST /conversations/{id}/messages). PLAN-04 complete (Hermes AI core, 5-provider router, circuit breaker, ReAct loop, gate; 16/16 tests; full suite 32/32). PLAN-08 complete (Vite+React SPA: all 11 steps; npm build passes with 0 TS errors; glassmorphism, Hermes orb, streaming chat, 4-column kanban, ApprovalModal, ConsoleDock, all pages implemented; self-hosted fonts/icons, no external origins). PLAN-05 complete (Knowledge RAG: sandbox file service, extraction, chunker with overlap, embeddings, pgvector HNSW search, optional rerank, idempotent indexing task, watcher daemon, files+kb routers, 3 test files; 9/9 steps).
